...
document.getElementById('PutdialogVehiculo').addEventListener('submit', function(event) {
    ...
    const PutdiVehiculoId = document.getElementById('PutdiVehiculoId').value;
    ...
    .then(data => {
        if (data) {
            alert('Vwhiculo actualizado con éxito.');
        } else {
    ...
    .catch(error => {
        ...
        alert('Hubo un problema al intentar actualizar el vehiculo.');
    }).finally(() => {
...
function registrarVehiculo() {
    const formData = {       
        ...
        potencia_cv: document.getElementById('potencia_cv').value,    
        precio_dia: document.getElementById('precio_dia').value
    };
